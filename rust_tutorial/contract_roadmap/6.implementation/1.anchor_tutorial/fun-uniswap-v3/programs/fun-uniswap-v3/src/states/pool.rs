...
impl PoolState {
    ...
    pub fn get_tick_array_offset(&self, tick_array_start_index: i32) -> Result<usize> {
        ...
        println!("tick_array_offset_in_bitmap: {}", tick_array_offset_in_bitmap);
        println!("tick_array_start_index: {}", tick_array_start_index);
        println!("TickArrayState::tick_count(self.tick_spacing): {}", TickArrayState::tick_count(self.tick_spacing));
        println!("tick_array_bit_map::TICK_ARRAY_BITMAP_SIZE: {}", tick_array_bit_map::TICK_ARRAY_BITMAP_SIZE);

        Ok(tick_array_offset_in_bitmap as usize)
    }

    fn flip_tick_array_bit_internal(&mut self, tick_array_start_index: i32) -> Result<()> {
        msg!("flip_tick_array_bit_internal 333");
        ...
        msg!("tick_array_bitmap: {:?}", tick_array_bitmap_arr);
        let mask: U1024 = U1024::one() << tick_array_offset_in_bitmap.try_into().unwrap();
        let mask_arr = mask.0;
        msg!("mask: {:?}", mask_arr);
        let new_bitmap_arr = tick_array_bitmap.bitxor(mask).0;
        self.tick_array_bitmap = new_bitmap_arr;
        msg!("self.tick_array_bitmap: {:?}", new_bitmap_arr);
        Ok(())
    }
    ...
    pub fn flip_tick_array_bit<'c: 'info, 'info>(
        ...
        if self.is_overflow_default_tickarray_bitmap(vec![tick_array_start_index]) {
            ...
            msg!("flip_tick_array_bit_internal 111");
    ...
    pub fn get_first_initialized_tick_array(
        ...
        //output intput parameters
        println!("tickarray_bitmap_extension: {:?}", tickarray_bitmap_extension);
        println!("zero_for_one: {:?}", zero_for_one);

        // Avoid borrowing a field from a packed struct by copying it to a local first
        let tick_array_bitmap = self.tick_array_bitmap;
        println!("self.tick_array_bitmap: {:?}", tick_array_bitmap);
        ...
        let (is_initialized, start_index) =
            if self.is_overflow_default_tickarray_bitmap(vec![self.tick_current]) {
                ...
                let bitmap_local = unsafe { core::ptr::read_unaligned(core::ptr::addr_of!(self.tick_array_bitmap)) };
...
pub mod pool_test {
    ...
    mod use_tickarray_bitmap_extension_test {
        ...
        use crate::states::tick_array_bitmap_extension_test::{
