...
pub mod tick_array_test {
    ...
    mod tick_array_layout_test {
        ...
        #[test]
        fn test_initialize_sets_fields_and_key_derivation() {
            let pool_id = Pubkey::new_unique();
            let start_index: i32 = 0; // valid: multiple of 60 * tick_spacing
            let tick_spacing: u16 = 60;
            println!("pool_id: {:?}", pool_id);


            // Ensure chosen start_index is valid for spacing
            assert!(TickArrayState::check_is_valid_start_index(start_index, tick_spacing));
            ...
            // PDA key matches expected seeds
            let (expected_pda, _bump) = Pubkey::find_program_address(
                &[TICK_ARRAY_SEED.as_bytes(), pool_id.as_ref(), &start_index.to_be_bytes()],
                &crate::id(),
            );

            println!("expected_pda: {:?}", expected_pda);
            println!("tick_array.key(): {:?}", tick_array.key());
            assert_eq!(tick_array.key(), expected_pda);
        }
    }
}
