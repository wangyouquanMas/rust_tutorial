...
pub fn add_liquidity<'b, 'c: 'info, 'info>(
    ...
    if flip_tick_upper {
        ...
        msg!("before_init_tick_count: {}", before_init_tick_count);
        tick_array_upper.update_initialized_tick_count(true)?;
        msg!("after_init_tick_count: {}", tick_array_upper.initialized_tick_count);
        if before_init_tick_count == 0 {
            pool_state.flip_tick_array_bit(
                tick_array_bitmap_extension,
...
pub fn update_position(
    ...
    //output tick_lower_state; tick_upper_state
    msg!("tick_lower_state: {:?}", tick_lower_state);
    msg!("tick_upper_state: {:?}", tick_upper_state);

    //output parameters 
    msg!("liquidity_delta: {}", liquidity_delta);
    msg!("pool_state: {:?}", pool_state);
    msg!("protocol_position_state: {:?}", protocol_position_state);
